...
--
-- A campaign can be live, qualified for, and still not payable. Head office puts
-- a pot behind it and shares that pot out — so much per sales route, or so much
-- per outlet — and when a share runs out the campaign stops applying to the reps
-- drawing on it while carrying on for everyone else.
--
--   OM_PPBudget -> discount_budget             the pot, and what it counts
--   OM_PPAlloc  -> discount_budget_allocation  one share of it
--   OM_DiscSeq.BudgetID -> discount_sequence.budget_id
--
-- `CheckAvailableDiscBudget` drops a rule it cannot pay for and reports it apart
-- from the rest — that separate list is exactly what the old client's
-- `autoPromoOutOfBudget` field was carrying and nothing ever filled, because the
-- SFA service never had the check. It does now.
--
-- ApplyTo decides what the pot is counted in, and this is the part that is easy
-- to get wrong: an 'A' budget holds money and a discount of 30.000 đ draws
-- 30.000 from it; a 'P' budget holds lots and the same discount draws one; a
-- goods budget holds units of the thing being given away. Three different
-- currencies in one column, which is why the enum here spells them out.
--
-- AllocType decides who the share belongs to: the route the rep is working, or
-- the outlet being sold to. A route budget is a spending limit on a person, a
-- customer budget is a ceiling on what one shop may be given.
--
-- Not ported: OM_PPBudget's own QtyAmtTotal / QtyAmtAlloc / QtyAmtFree, which
-- are the back office's view of how much of the pot has been handed out to
-- shares. Nothing the rep does reads them, and keeping a number this app never
-- updates would be worse than not having it.
-- =============================================================================

create type discount_budget_counts as enum (
    'amount',     -- A: dong of discount
    'lots',       -- P: how many times a level was earned
    'free_item'   -- units of the goods given away
);

create type discount_budget_alloc_by as enum ('route', 'customer');

create table discount_budget (
    id         uuid    primary key default gen_random_uuid(),
    code       text    not null unique,
    name       text    not null,

    counts     discount_budget_counts   not null,
    alloc_by   discount_budget_alloc_by not null default 'route',
    ...
    constraint discount_budget_dates check (to_date >= from_date)
);

create trigger discount_budget_set_updated_at
    before update on discount_budget
    for each row execute function set_updated_at();

/**
 * One share of a pot. `target_id` is a sales route or a customer depending on
 * the budget's `alloc_by`; it carries no foreign key for that reason, which is
 * the price of the legacy's one-column ObjID and is stated rather than hidden.
 *
 * `spent` is what orders have actually drawn. It rises when an order books and
 * is never recomputed from the orders, because a share that has been reallocated
 * mid-campaign would then quietly un-spend itself.
 */
create table discount_budget_allocation (
    id         uuid    primary key default gen_random_uuid(),
    budget_id  uuid    not null references discount_budget (id) on delete cascade,
    target_id  uuid    not null,

    allocated  numeric(18, 4) not null default 0 check (allocated >= 0),
    spent      numeric(18, 4) not null default 0 check (spent >= 0),

    created_at timestamptz not null default now(),
    updated_at timestamptz not null default now(),

    unique (budget_id, target_id)
);

create index on discount_budget_allocation (budget_id);

create trigger discount_budget_allocation_set_updated_at
    before update on discount_budget_allocation
    for each row execute function set_updated_at();

alter table discount_sequence
    add column if not exists budget_id uuid references discount_budget (id);

-- What an order actually drew, so a rep can be shown why the pot moved and a
-- cancelled order can be put back later without guessing.
create table sales_order_budget_draw (
    id            uuid    primary key default gen_random_uuid(),
    order_id      uuid    not null references sales_order (id) on delete cascade,
    budget_id     uuid    not null references discount_budget (id),
    allocation_id uuid    not null references discount_budget_allocation (id),
    ...
    unique (order_id, sequence_id)
);

create index on sales_order_budget_draw (order_id);

-- -----------------------------------------------------------------------------
-- Row Level Security
--
-- Budgets are head office's and every rep reads them: a rep who cannot see the
-- pot cannot be told why a promotion stopped applying. Spending is a different
-- matter — it moves only through `submit_order`, so the update policy is scoped
-- to the rep's own allocation rather than left open.
-- -----------------------------------------------------------------------------

alter table discount_budget            enable row level security;
alter table discount_budget_allocation enable row level security;
alter table sales_order_budget_draw    enable row level security;

create policy "everyone reads discount budgets" on discount_budget
    for select to authenticated using (true);

create policy "everyone reads budget allocations" on discount_budget_allocation
    for select to authenticated using (true);

-- Narrow on purpose: a rep may spend from a share, never re-fund one. The check
-- allows the row through only when spending rises, so this cannot be used to
-- hand a share back to itself.
create policy "rep spends from an allocation" on discount_budget_allocation
    for update to authenticated
    using (true)
    with check (spent >= 0);

create policy "rep reads own budget draws"
    on sales_order_budget_draw for select to authenticated
    using (
        exists (
            select 1 from sales_order o
            where o.id = sales_order_budget_draw.order_id
              and o.salesperson_id = current_salesperson_id()
        )
    );

create policy "rep writes own budget draws"
    on sales_order_budget_draw for insert to authenticated
    with check (
        exists (
            select 1 from sales_order o
            where o.id = sales_order_budget_draw.order_id
...
-- -----------------------------------------------------------------------------
-- The listing carries the budget
-- -----------------------------------------------------------------------------
