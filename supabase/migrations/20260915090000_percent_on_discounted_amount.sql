-- =============================================================================
-- Phần trăm tính trên số tiền sau giảm
--
-- OM10100 takes a percentage on `Amt`, which is the order line's amount as it
-- stands — and it stands lower once an earlier rule has written a discount onto
-- it. `UpdateLineDiscAmt` and `UpdateGroupDiscByInvtIDV2` do that writing, line
-- by line, as each rule fires; a later percentage rule then reads the reduced
-- figure back. For the document scope it reads `LineAmt - VolDiscAmt`, which is
-- the same idea at order level.
--
-- This engine had been taking percentages on the gross. On an order of ten
-- million where a line rule has already given back five hundred thousand, three
-- per cent is 285.000 rather than 300.000 — and, more than the fifteen thousand,
-- it decides whether the order in which campaigns fire changes what they are
-- worth. It does, and now it does here too.
--
-- Two things this needs that the engine did not have:
--
--   * a ledger of what has already been discounted off each line, since the
--     working cart is reduced by *thresholds consumed*, which is a different
--     number from money given back;
--   * an order-scope percentage measured on the whole order rather than on the
--     remainder the threshold walk happens to be holding, which is what
--     OM10100's `amt = docAmt` says and what this got wrong.
--
-- The split is proportional: a discount earned by a group of products comes off
-- those products in the ratio they contributed, which is the only division that
